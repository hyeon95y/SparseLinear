...
class GrowConnections(torch.autograd.Function):
    ...
    def forward(ctx,input,weights, k, indices, features):
        ...
        if len(output_shape) == 1: input = input.view(1, -1)
    ...
    def backward(ctx, grad_output):
        # In the backward pass of grow, get top k of input values, top k of the grad_output values 
        # and use top k of grad_weight values - can work at the moment for 2d inputs only
        input, weights, indices  = ctx.saved_tensors
        k = ctx.in1
...
class SparseLinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True, sparsity=0.9, connectivity=None, small_world=False, dynamic=False):
    ...
    def forward(self, input):
        ...
        if self.train and self.dynamic and self.itr_count < self.Tend and self.itr_count%self.deltaT==0:
            ...
            output = torch_sparse.spmm(self.indices, self.weights, self.out_features, self.in_features, input.t()).t()
            if self.bias is not None:
                output += self.bias
